...
export default function Sidebar({ friends, onAddFriend, onSelectFriend, selected, signedIn, currentUser, onSignIn, onSignOut, theme, toggleTheme, customBg, customText, uiStyle, setCustomBg, setCustomText, setUiStyle, resetSettings }: Props) {
  ...
  const NavItem = ({ icon: Icon, active, bottom }: { icon: any, active?: boolean, bottom?: boolean }) => (
    <div style={{
      padding: 10,
      display: 'flex',
      ...
      alignItems: 'center',
      cursor: 'pointer',
      color: active ? '#fff' : '#aeaeae',
      position: 'relative',
      marginTop: bottom ? 'auto' : 0
    }}>
      {active && <div style={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', height: 16, width: 3, background: '#25D366', borderRadius: '0 4px 4px 0' }} />}
  ...
  return (
    <div className="sidebar" style={{ display: 'flex', flexDirection: 'row', padding: 0, background: '#2b2b2b', borderRight: '1px solid #3b3b3b', width: 350 }}>
      ...
      <div style={{
        ...
        {/* Top Icons */}
        <NavItem icon={Menu} />
        <NavItem icon={MessageSquare} active />
        <NavItem icon={Phone} />
        <NavItem icon={CircleDashed} />
        ...
        <div onClick={() => setShowSettings(true)} style={{ cursor: 'pointer' }}><NavItem icon={Settings} /></div>
        <div onClick={signedIn ? onSignOut : onSignIn} title={signedIn ? "Sign Out" : "Sign In"}>
          {signedIn && currentUser ? (
            <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#00a884', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, margin: '10px 0' }}>
              {currentUser.name[0]}
            </div>
          ) : (
      ...
      {/* Chat List Column */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#2b2b2b' }}>

        {/* Header 'Chats' */}
        <div style={{ height: 60, padding: '0 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: '#e1e1e1' }}>Chats</h2>
          <div style={{ display: 'flex', gap: 16, color: '#aeaeae' }}>
            <MessageSquarePlus size={20} style={{ cursor: 'pointer' }} onClick={() => setShowAdd(s => !s)} />
            <div style={{ position: 'relative' }}>
              <Menu size={20} style={{ transform: 'rotate(90deg)' }} /> {/* Simulating Filter icon */}
            </div>
          </div>
        </div>
        ...
        {/* List */}
        <div className="friends-list" style={{ background: 'transparent' }}>
          {friends.map(f => (
            <div key={f.email} className="friend" onClick={() => onSelectFriend(f)} style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: 2 }}>
                ...
                <div className="small" style={{ fontSize: 13, color: '#aaa', display: 'flex', alignItems: 'center', gap: 4 }}>
                  <span style={{ color: '#53bdeb' }}>✓✓</span> You: https://vt.tiktok...
                </div>
              </div>
            </div>
          ))}
